...
(() => {
  ...
  function trend(){
    ...
    if(!market)return missing('行情暂时没有取到，图表不会用模拟数值填充。');
    ...
    const grid=[high,(high+low)/2,low].map(v=>`<path d="M8 ${y(v)}H267" class="nl-chart-grid"/>${text(317,y(v)+3,number(v,v>=10000?0:2),'text-anchor="end"')}`).join('');
    ...
    const figure=svg(`${report.stock.name}最近 ${n} 个交易日价格走势，单位${market.currency}，${market.basis}；下方是相对成交量。`,
  ...
  function cachePanel(updating){
    const sections=report.sections,available=Object.values(sections).filter(s=>s?.data),stale=available.some(s=>s.stale||s.error);
    const title=updating?'正在更新，先看已保存数据':!available.length?'资料暂未取到':stale?'部分资料尚未更新':available.every(s=>s.via==='cache')?'已读取保存的数据':'数据已整理';
    return `<div class="nl-cache-toolbar"><span class="${stale?'is-stale':''}"><i></i>${title}</span><button type="button" id="refresh-data" ${updating?'disabled':''}>${updating?'更新中…':'刷新数据'}</button></div><details class="nl-cache-details"><summary>数据时间与缓存说明</summary><p>当前浏览器保存最近查看过的数据。量价 5 分钟、新闻 15 分钟、公司资料 24 小时内优先复用；过期后查看时更新。手动刷新间隔至少 30 秒。</p>${Object.entries({market:'量价数据',profile:'公司资料',news:'个股新闻',industryNews:'行业资讯'}).filter(([key])=>key in sections).map(([key,label])=>{const s=sections[key];return `<div><span>${label}</span><span>${s?.data?`${time(s.fetchedAt)} 获取${s.stale?' · 已过期':s.via==='cache'?' · 已缓存':' · 新获取'}`:'暂缺'}${s?.error?' · 更新未成功':''}</span></div>`;}).join('')}<p>获取时间为北京时间；不代表平台内容刚刚更新。缓存只在本设备、当前浏览器中复用，清理网站数据后会移除。${window.NiulaiCache.info().persistent?'':'当前浏览器无法持久保存，仅在本次打开期间复用。'}</p></details>`;
  }
  function render(stock,marketLabel,data,{updating=false}={}){
    report=data;metrics=window.NiulaiData.calculate(report.sections.market?.data);view={days:20,point:19,indicator:'MACD'};
    const m=report.sections.market?.data,q=m?.quote;
    const headline=metrics.distance===null?'先把这家公司看明白':`价格${metrics.distance>=0?'高于':'低于'}近期平均水平`;
    const quote=q?`<div class="nl-live-quote"><strong>${number(q.price)}</strong><span class="${q.change>=0?'nl-positive':'nl-negative'}">${signed(q.change)}</span><small>${escape(m.currency)} · ${q.delayed?'可能延迟':'最新快照'}</small></div><p class="nl-data-stamp">行情时间 ${escape(q.time)} · ${escape(m.timezone)}</p>`:'';
    const old=m && Date.now()-Date.parse(m.bars.at(-1).date)>10*86400000;
    return `<article class="nl-report nl-visual-report"><div class="nl-report-stock"><h3>${escape(stock.name)}</h3>${marketLabel}<span class="nl-report-code">${escape(stock.code)}</span></div>${quote}${cachePanel(updating)}${old?'<p class="nl-coverage-note">行情日期较早，请核对是否停牌或来源尚未更新。</p>':''}${!data.symbol?'<p class="nl-coverage-note">目录收录了这只股票，但暂未验证其腾讯行情代码，当前不展示数据。</p>':''}<div class="nl-visual-verdict"><div><span>牛来帮你读数据 · 规则解读</span><h4>${headline}</h4><p>看生意 · 读消息 · 看交易变化</p></div><img src="assets/niulai.jpg" alt="" width="1254" height="1254"></div><section class="nl-chart-panel">${caption('01','行业和生意怎么样')} ${industry()}</section><section class="nl-chart-panel">${caption('02','最近发生了什么')}<div id="nl-news-chart">${newsChart()}</div></section><section class="nl-chart-panel">${caption('03','价格和成交量怎么变',`<div class="nl-chart-tabs" role="group" aria-label="走势时间范围"><button type="button" data-trend-days="20" class="is-active" aria-pressed="true">20 日</button><button type="button" data-trend-days="60" aria-pressed="false">60 日</button></div>`)}<div id="nl-trend-chart">${trend()}</div>${signals()}${m?`<details class="nl-more-data nl-technical-details"><summary>再看看技术指标</summary><div class="nl-indicator-tabs" role="group" aria-label="切换技术指标"><button type="button" data-indicator="MACD" class="is-active" aria-pressed="true">涨跌速度 · MACD</button><button type="button" data-indicator="RSI" aria-pressed="false">涨跌强弱 · RSI</button></div><div id="nl-technical-chart">${technical()}</div></details>`:''}${stamp(report.sections.market,'腾讯行情 · 图表及指标由本站计算')}</section><div class="nl-report-source"><a href="${escape(window.NiulaiData.sourceURL(stock))}" target="_blank" rel="noopener noreferrer">在腾讯自选股核对资料 ↗</a><span>公开来源可能延迟或缺失</span></div><div class="nl-report-actions"><button type="button" id="ask-another">换一只股票问问</button></div></article>`;
  }
